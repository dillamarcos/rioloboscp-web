...
<div x-data="{ open:false, userMenu:false, panelOpen: false }" x-cloak>
    ...
    <!-- HEADER -->
    <div class="fixed top-0 left-0 right-0 z-50 w-full h-16 md:h-20 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-200 flex items-center justify-between 
        border border-b-1 border-gray-200 dark:border-gray-700 shadow-md dark:shadow-[0_4px_12px_rgba(0,0,0,0.6)] mb-4 md:mb-6 px-4 md:px-10 transition-colors duration-300">

        <!-- LOGO -->
        <div class="flex items-center">
            <a href="{{ route('home') }}">
                <img src="{{ asset('images/escudo_nav.png') }}" alt="Logo" class="cursor-pointer h-12 md:h-16 lg:h-18 hover:scale-105 transition">
            </a>

            <!-- HAMBURGUESA -->
            <button @click="open = !open" class="cursor-pointer lg:hidden p-2 text-gray-700 dark:text-gray-200 text-2xl flex items-center justify-center w-9 h-9 ml-2 
                        hover:text-indigo-500 transition hover:scale-110 active:scale-95">
        ...
        @php
            function panelActive($route) {
                return request()->routeIs($route) ? 'bg-indigo-50 dark:bg-indigo-500/10 border-l-4 border-indigo-600 text-indigo-700 dark:text-indigo-400 font-bold rounded-md'
        ...
        <!-- NAV DESKTOP -->
        <nav class="hidden lg:flex items-center space-x-4 lg:space-x-6 xl:space-x-12 text-xs lg:text-sm xl:text-base">
        ...
        <div class="flex items-center space-x-3 ml-3 lg:ml-0">
                ...
                @if(auth()->user()->rol === 'admin' || auth()->user()->rol === 'editor')
                    ...
                    <button @click="panelOpen = !panelOpen" 
                            class="mr-2 text-xl transition flex items-center justify-center hover:scale-105
                                {{ $isPanelRoute ? 'text-indigo-500 hover:text-indigo-400' : 'text-gray-700 dark:text-gray-200 hover:text-indigo-500' }}">
            ...
            <!-- INVITADO -->
            @guest
                <button @click="userMenu = !userMenu" class="cursor-pointer border border-gray-300 dark:border-gray-600 rounded-full text-indigo-500 hover:bg-gray-100 dark:hover:bg-gray-800 
                                flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 transition hover:scale-110 active:scale-95">
            ...
            <div x-show="userMenu" x-cloak @click.away="userMenu = false" class="absolute right-2 md:right-4 top-14 md:top-16 
                w-25 md:w-40 bg-white dark:bg-gray-800 shadow-lg dark:shadow-black/50 rounded-lg  border border-gray-200 dark:border-gray-700 z-50 mt-0 md:mt-2">

                <a href="{{ route('login') }}" class="block px-4 py-2 text-xs md:text-base hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200">
                    Iniciar sesión
                </a>

                <a href="{{ route('register') }}" class="block px-4 py-2 text-xs md:text-base hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200">
                    Registrarse
                </a>
            </div>
            ...
            @auth
                <div class="relative" x-data="{ userMenu: false }">
                    ...
                    <button @click="userMenu = !userMenu"
                        class="relative cursor-pointer flex items-center justify-center rounded-full bg-gradient-to-br from-indigo-600 via-indigo-700 to-indigo-900 text-white shadow-md dark:shadow-black/60
                            w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 hover:shadow-lg hover:shadow-indigo-500/30 transition-all duration-300 hover:scale-105 active:scale-95
                            ring-2 ring-white dark:ring-gray-800 ring-offset-2 dark:ring-offset-gray-900">

                        @if(auth()->user()->foto_perfil)
                            <img src="{{ asset('storage/' . auth()->user()->foto_perfil) }}" class="rounded-full w-full h-full object-cover">
                        @else
                    ...
                    <div x-show="userMenu"
                        ...
                        <div class="">
                            @if(auth()->user()->socio)
                                <a href="{{ route('socio.show') }}"
                                    class="flex items-center gap-3 px-4 py-2.5 text-sm transition-all
                                        {{ request()->routeIs('socio.show') ? 'bg-indigo-50 dark:bg-indigo-500/10 border-l-4 rounded-md border-indigo-600 text-indigo-700 dark:text-indigo-400 font-bold' 
                            ...
                            <a href="{{ route('profile.edit') }}"
                                class="flex items-center gap-3 px-4 py-2.5 text-sm transition-all
                                    {{ request()->routeIs('profile.edit') ? 'bg-indigo-50 dark:bg-indigo-500/10 border-l-4 rounded-md border-indigo-600 text-indigo-700 dark:text-indigo-400 font-bold' 
                                            : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700/50 border-l-4 border-transparent' }}">
    ...
    <aside x-show="panelOpen"
        ...
        <div class="p-5">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button @click="panelOpen = false" class="text-gray-500 hover:text-red-500 transition text-lg">
                ...
                @auth
                    ...
                    <div x-data="{ openSocios: {{ request()->routeIs('panel.instructor.socios*','panel.instructor.solicitudes*') ? 'true' : 'false' }} }">
                        ...
                        <button @click="openSocios = !openSocios"
                            class="w-full flex justify-between items-center px-3 py-2 transition-all font-semibold border-l-4 border-transparent rounded-md text-gray-700 dark:text-gray-300
                                hover:bg-gray-100 dark:hover:bg-gray-700">

                            <span>Socios</span>

                            <i class="fa-solid fa-chevron-down text-xs transition" :class="{ 'rotate-180': openSocios }"></i>
                        ...
                        <div x-show="openSocios"
                            ...
                            <a href="{{ route('panel.instructor.socios') }}" class="block px-3 py-1 transition-all rounded-md {{ panelActive('panel.instructor.socios*') }}">
                                Todos los socios
                            </a>

                            <a href="{{ route('panel.instructor.solicitudes') }}" class="block px-3 py-1 transition-all rounded-md {{ panelActive('panel.instructor.solicitudes*') }}">
                                Solicitudes
                            </a>
                    ...
                    <!-- RESTO -->
                    <a href="{{ route('panel.instructor.partidos') }}" class="block px-3 py-2 transition-all font-semibold {{ panelActive('panel.instructor.partidos*') }}">
                        Partidos
                    </a>

                    <a href="{{ route('panel.instructor.jugadores') }}" class="block px-3 py-2 transition-all font-semibold {{ panelActive('panel.instructor.jugadores*') }}">
                        Jugadores
                    </a>

                    <a href="{{ route('panel.instructor.equipos') }}" class="block px-3 py-2 transition-all font-semibold {{ panelActive('panel.instructor.equipos*') }}">
                        Equipos y temporadas
                    </a>

                    <a href="{{ route('panel.instructor.productos') }}" class="block px-3 py-2 transition-all font-semibold {{ panelActive('panel.instructor.productos*') }}">
                        Productos
                    </a>

                    <a href="{{ route('panel.instructor.noticias') }}" class="block px-3 py-2 transition-all font-semibold {{ panelActive('panel.instructor.noticias*') }}">
                        Noticias
                    </a>
